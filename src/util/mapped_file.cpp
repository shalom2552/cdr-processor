...
MappedFile::MappedFile(const std::string& path)
{
    ...
    if (fd < 0) {
        logError("[MappedFile] open failed: " + path);
        return;
    }

    struct stat st{};
    if (fstat(fd, &st) != 0) {
        logError("[MappedFile] fstat failed: " + path);
    } else if (!S_ISREG(st.st_mode)) {
        logError("[MappedFile] not a regular file: " + path);
    } else if (st.st_size == 0) {
        logWarn("[MappedFile] empty file: " + path);
        m_ok = true; // empty file
    } else {
        void* p = mmap(nullptr, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
        if (p == MAP_FAILED) {
            logError("[MappedFile] mmap failed: " + path);
        } else {
