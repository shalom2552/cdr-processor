...
bool rpc_ok(amqp_rpc_reply_t reply, const char* what)
{
    if (reply.reply_type == AMQP_RESPONSE_NORMAL) {
        ...
        cdrp::logError("RabbitConn", std::string(what) + " failed");
        return false;
    }
}
...
bool RabbitConn::open(const std::string& url, const std::string& queue, uint16_t prefetch)
{
    close();
    ...
    if (amqp_socket_open(socket, info.host, info.port) != AMQP_STATUS_OK) {
        logError("RabbitConn", "connection failed: " + std::string(info.host));
        close();
        return false;
    }

    const auto login = amqp_login(m_conn, info.vhost, 0, AMQP_DEFAULT_FRAME_SIZE, 0, AMQP_SASL_METHOD_PLAIN, info.user, info.password);
    ...
    amqp_basic_qos(m_conn, kChannel, 0, prefetch, 0);
    ...
    logInfo("RabbitConn", "consuming: " + queue + " prefetch " + std::to_string(prefetch));
    return true;
}
...
RabbitConn::Status RabbitConn::consume(Message& out, int timeout_ms)
{
    ...
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
        ...
        logError("RabbitConn", "consume failed");
        return Status::FAIL;
    }
