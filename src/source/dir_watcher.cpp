...
constexpr std::string_view kComponent = "DirWatcher";
    ...
    , m_target_dir(target_dir)
{
    ...
    if (m_fd < 0) {
        logError(kComponent, "inotify_init failed: " + std::string(std::strerror(errno)));
        return;
    }
    m_wd = inotify_add_watch(m_fd, m_source_dir.c_str(), IN_MOVED_TO);
    if (m_wd < 0) {
        logError(kComponent, "cannot watch: " + m_source_dir + ": " + std::strerror(errno));
        close(m_fd);
        m_fd = -1;
        return;
    }
    ...
    if (m_event_fd < 0) {
        logError(kComponent, "eventfd failed: " + std::string(std::strerror(errno)));
        close(m_fd);
        m_fd = -1;
        return;
    }
    logInfo(kComponent, "watching: " + m_source_dir);
...
bool DirWatcher::next_file(std::string& path)
{
    ...
    logDebug(kComponent, "next_file: " + path);
    return true;
}
...
void DirWatcher::wake()
{
    ...
    if (write(m_event_fd, &one, sizeof(one)) != sizeof(one)) {
        logWarn(kComponent, "wake failed: " + std::string(std::strerror(errno)));
    }
}
...
bool DirWatcher::claim(const std::string& file_name)
{
    ...
    if (std::rename(source.c_str(), dest.c_str()) != 0) {
        ...
        if (errno == ENOENT) {
            logDebug(kComponent, "already claimed: " + file_name);
        } else {
            logWarn(kComponent, "claim failed: " + file_name + ": " + std::strerror(errno));
        }
        return false;
    }
    m_queue.push_back(dest);
    logInfo(kComponent, "claim: " + file_name);
    if (m_queue.size() > kBacklogAlert) {
        logDebug(kComponent, "backlog of " + std::to_string(m_queue.size()) + " files");
    }
    return true;
}
...
void DirWatcher::sweep(const std::string& dir, bool claimed)
{
    ...
    if (ec) {
        logError(kComponent, "scan failed: " + dir + ": " + ec.message());
        return;
    }
...
bool DirWatcher::read_events()
{
    ...
    if (poll(fds, 2, -1) < 0) {
        ...
        logError(kComponent, "poll failed: " + std::string(std::strerror(errno)));
        return false;
    }
    ...
    if (fds[1].revents & POLLIN) {
        ...
        logInfo(kComponent, "woken: shutdown requested");
        return false;
    }
    ...
    if (n <= 0) {
        logError(kComponent, "inotify read failed: " + std::string(std::strerror(errno)));
        return false;
    }
